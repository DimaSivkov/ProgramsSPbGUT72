...
while flag == "+":
    ...
    if currency == 1:
        money = int(input("Введите количество евро которые вы ходите взять в кердит: "))
        if money > 0:
        ...
        money = int(input("Введите количество долларов которые вы ходите взять в кердит: "))
        if money > 0:
            rate = int(input("Введите процентную ставку = "))
            period = int(input("Введите период на который вы берете кредит = "))
            result = round((((money*rate)/100)+money),0)
            mounth = round(result/period,2)
            print("Вам надо будет отдать банку: ",result,"Долларов","или по",mounth,"Долларов в течении",period,"месяцев")
        else:
            print("Введите сумму побольше чем 0 :) ")
